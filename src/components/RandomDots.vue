<template>
    <div class="position--relative height--full">
      <div class="padding--20 height--full margin--40">
        <div ref="main" class="position--center position--absolute margin--40">
          <div ref="box" class="box" v-for="(box, index) in boxes" :key="index" :style="box.style" :class="{ 'transition': isTransition }"></div>
        </div>
      </div>
    </div>
  </template>
    ...
    methods: {
      ...
      updateWindowSize() {
        ...
        const breakpoints = [
          ...
          { width: 350, size: 80 },    // For screens below 600px width, box size is set to 80px
          { width: 768, size: 120 }, // For screens of 600px and above, box size is set to 120px
          { width: 1440, size: 200 }, // For screens of 1024px and above, box size is set to 160px
  ...
  .box {
    ...
    border-radius: 50%;
    position: absolute;
    transition: left 0.5s ease, top 0.5s ease; /* CSS transition for left and top properties */
  }
  

  @media (max-width: 350px) {
    /* CSS styles for screens below 600px width */
    .box {
      width: 20px;
      height: 20px;
    }
  ...
  @media (min-width: 768px) {
    /* CSS styles for screens of 600px and above */
    .box {
      width: 80px;
      height: 80px;
    }
  ...
  @media (min-width: 1440px) {
    /* CSS styles for screens of 1024px and above */
    .box {
      width: 120px;
      height: 120px;
    }
  ...
  .transition {
    transition: left 0.5s ease, top 0.5s ease; /* CSS transition for left and top properties during animation */
    }
  
  /* Add more media queries and corresponding box sizes as needed */
  </style>
